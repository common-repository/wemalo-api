...
class WemaloAPIBasic {
	...
	/**
	 * checks whether auth-key was set in header and matches option
	 *
	 * @param WP_REST_Request $request
	 * @throws WemaloAPIException
	 */
	protected function check_authentication( WP_REST_Request $request ) {
	 ...
	 */
	protected function generate_output( array|string $data = null, string $message = '', bool $success = true, int $code = 200, string $token = '' ): WP_REST_Response {
		$result = array();
		if ( ! empty( $data ) ) {
			$result['data'] = $data;
			if ( $token !== '' ) {
	 ...
	 * @return WP_REST_Response
	 */
	protected function handle_error( WemaloAPIException|Exception $e ): WP_REST_Response {
		$code = 500;
		if ( $e instanceof WemaloAPIException ) {
			$code = $e->get_return_code();
		}
		return $this->generate_output( null, $e->getMessage(), false, $code );
	}
}
